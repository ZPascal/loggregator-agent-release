...
var _ = Describe("LegacySyslogBindingCache", func() {
	var (
		logger       = log.New(GinkgoWriter, "", log.LstdFlags)
		metricClient *metricsHelpers.SpyMetricsRegistry
		config       app.Config
	...
	BeforeEach(func() {
		r := legacyResults{
			"app-id-1": legacyAppBindings{
				Drains:   []string{"syslog://drain-a", "syslog://drain-b"},
				Hostname: "org.space.app-name",
			},
			"app-id-2": legacyAppBindings{
...
type legacyResults map[string]legacyAppBindings
type legacyAppBindings struct {
...
func (f *fakeCC) serveWithResults(w http.ResponseWriter, r *http.Request) {
	if f.legacy {
		resultData, err := json.Marshal(struct {
			Results legacyResults `json:"results"`
		}{
			Results: f.legacyResults,
		})
		if err != nil {
			w.WriteHeader(500)
			return
